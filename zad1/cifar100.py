...
def summarize_diagnostics(history, counter:int=0 , data_name:str = "cifar100", run:int=0):
    # plot loss
    pyplot.subplot(211)
    ...
    #filename = f"{data_name}\{str(run)}\plot\{data_name}_{str(counter)}"
...
def create_log(layer_number:int=1,
    ...
    #my_data_file = open(f"{data_name}\{str(run)}\plot\{data_name}_{str(counter)}.txt")
